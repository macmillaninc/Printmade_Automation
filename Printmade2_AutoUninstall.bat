...
set uninstaller="C:\Program Files (x86)\Printmade2\unins000.exe"
set uninstaller86="C:\Program Files\Printmade2\unins000.exe"

rem Uninstaller start code
rem START "" "C:\Program Files (x86)\Printmade2\unins000.exe"

if exist %uninstaller% (

echo automatically uninstalling printmade2...
echo stand by for relay...

START "" "C:\Program Files (x86)\Printmade2\unins000.exe"
...
goto :last
	...
	echo There is no Printmade2 on this system.
	echo X86 prox checking...
		if exist %uninstaller86% (
			echo X86 Confirmed.
			START "" "C:\Program Files\Printmade2\unins000.exe"
			ping -n 3 -w 1 127.0.0.1 > NUL
			%SendKeys% "Y"
			ping -n 5 -w 1 127.0.0.1 > NUL
			%SendKeys% "{ENTER}"
			ping -n 3 -w 1 127.0.0.1 > NUL
			%SendKeys% "{ENTER}"
			) else ( echo Printmade2 does not exist.
					goto :last) )
